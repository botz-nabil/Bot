...
offline = false
targetpc = '0'
ownerNumber = ["6288989575662@s.whatsapp.net","6288989575662@s.whatsapp.net","6281938835391@s.whatsapp.net","6281938835391@s.whatsapp.net"]
fake = '𝙸𝚖 BotzNabil'
fake1 = '_*© NABIL*_'
numbernye = '0'
waktu = '-'
...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        //FAKEREPLY PRODUCT
            const ftoko = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "60139571124@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./stik/thumb.jpeg`)},"title": `OPEN BO KAK?\n${pushname}`,"description": "𝙸 𝙰𝙼 𝙽𝙳𝙰𝙰𝙱𝙾𝚃𝚉", "currencyCode": "IDR","priceAmount1000": "9999999999","retailerId": "𝙸 𝙰𝙼 𝙽𝙳𝙰𝙰𝙱𝙾𝚃𝚉","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
            //FAKE KONTAK
            const fkontak = { 
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `6283162388082@s.whatsapp.net`, ...(from ? { remoteJid: "6285878313791-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '-99999', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            ...
            //FAKEREPLY GIF
            const fgif = {
                  key: {fromMe: false,participant: `6283162388082@s.whatsapp.net`, ...(from ? { remoteJid: "6285878313791-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}} 
...
switch (command) {
	...
	run = process.uptime() 
    teks = `${kyun(run)}`
gambur = 'https://a.uguu.se/BgczgryY.jpg'
...
xdev.sendMessage(from,'𝙽𝚒𝚑 𝙾𝚠𝚗𝚎𝚛 𝙺𝚞 𝙺𝚊𝚔, 𝙹𝚊𝚗 𝙶𝚊𝚕𝚊𝚔 𝙶𝚊𝚕𝚊𝚔 𝙽𝚊𝚗𝚝𝚒 𝙺𝚞 𝙱𝚊𝚗𝚗𝚎𝚍 𝙽𝚊𝚗𝚐𝚎𝚜𝚜^_^',text,{quoted: hehe})
}
if (button == '⎙ INSTAGRAM') {
console.log('INSTAGRAM')
xdev.sendMessage(from, `*Stalkerin Aja Tuh*\n*https://instagram.com/nab.nab_27*`, text, {quoted: dev})
}
if (button == '🔍 NEKOPOI') {
console.log('YOUTUBE')
xdev.sendMessage(from, `*Subscribe Chanel Owner Ya*\n_https://youtube.com/channel/UCGQzOIGQIkxLGjKzyH2thwg_`, text, {quoted: dev})
}
if (budy.startsWith('$')){
if (!dev.key.fromMe && !isOwner) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`@𝙽𝚊𝚗𝚍𝚊 𝚂𝚝𝚢𝚕𝚎:~ ${err}`)
if (stdout) {
reply(stdout)
}
})
